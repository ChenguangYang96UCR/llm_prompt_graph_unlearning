...
class Logger(object):
    ...
    def print_statistics(self, run=None, mode='max_acc'):
        if run is not None:
            ...
            self.logger.debug(f'Run {run + 1:02d}:')
            self.logger.debug(f'Highest Train: {result[:, 0].max():.2f}')
            self.logger.debug(f'Highest Valid: {result[:, 1].max():.2f}')
            self.logger.debug(f'Highest Test: {result[:, 2].max():.2f}')
            self.logger.debug(f'Chosen epoch: {ind}')
            self.logger.debug(f'Final Train: {result[ind, 0]:.2f}')
            self.logger.debug(f'Final Test: {result[ind, 2]:.2f}')
            ...
            self.logger.debug(f'All runs:')
            r = best_result[:, 0]
            self.logger.debug(f'Highest Train: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 1]
            self.logger.debug(f'Highest Test: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 2]
            self.logger.debug(f'Highest Valid: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 3]
            self.logger.debug(f'  Final Train: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 4]
            self.logger.debug(f'   Final Test: {r.mean():.2f} ± {r.std():.2f}')
