...
def edge_weight_func(features_array):
    ...
    for i in range(features_array.shape[0]-1):
        for j in range(i+1, features_array.shape[0]):
            ...
            cos_sim = np.dot(sources, targets) / (np.linalg.norm(sources) * np.linalg.norm(targets))
            edge_weight_mat[i, j] = (cos_sim + 1) / 2
            edge_weight_mat[j, i] = edge_weight_mat[i, j] 
...
def simplicial_complex_dgm(target_matrix):
    rips_complex = gd.RipsComplex(distance_matrix=target_matrix, max_edge_length=1.0)
    simplex_tree = rips_complex.create_simplex_tree(max_dimension=2)
    
    diag = simplex_tree.persistence(homology_coeff_field=2, min_persistence=0)
    if len(diag)!= 0:
        res = np.zeros(shape=(np.array(diag, dtype=object)[:,1].shape[0], 2), dtype= np.float32)
        for i in range(np.array(diag, dtype=object).shape[0]):
            if np.isfinite(np.array(diag, dtype=object)[:,1][i][1]):
                res[i, :] = np.array(diag, dtype=object)[:, 1][i]
    else:
        res = np.array([])
    return res
